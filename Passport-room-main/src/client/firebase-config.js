...
/** Appends a new child under `path` and returns its generated key. */
export async function dbPost(path, value, { auth } = {}) {
  const res = await fetch(url(path, auth), {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify(value),
  });
  if (!res.ok) throw new Error(`firebase append ${path} failed (${res.status})`);
