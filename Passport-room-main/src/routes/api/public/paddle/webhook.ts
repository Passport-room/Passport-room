      ...
      POST: async ({ request }) => {
        ...
        try {
          ...
          if (SUBSCRIPTION_EVENTS.has(type)) {
            ...
            // A missing device id must never block a paid membership: the
            // buyer's email anchors the record instead.
            const grants =
              result === "stored" &&
              Boolean(wanted) &&
              Boolean(deviceId || email) &&
          ...
          const wanted = expectedPriceId();
          if (!wanted) {
            // A missing price id must not swallow a real payment: the event is
            // still honoured, and the misconfiguration is logged loudly.
            console.error("[paddle-webhook] PADDLE_PRICE_ID is not configured");
          }
          if (!paidEventSellsOurPlan(collectPriceIds(data), collectProductIds(data))) {
            console.warn("[paddle-webhook] paid event does not match the configured plan", {
              prices: collectPriceIds(data),
              products: collectProductIds(data),
            });
            return Response.json({ ok: true, skipped: "price id does not match" });
          }

          const deviceId = customDeviceId(data);
          // The buyer's normalized email is enough to create the membership,
          // so a checkout without a device id still activates Pro.
          const paidEmail =
            customEmail(data) ?? (await fetchPaddleCustomerEmail(data.customer_id ?? null));
          if (!deviceId && !paidEmail) {
            return Response.json({ ok: true, skipped: "no device id and no email" });
          }
