...
export async function canUseWebGPU() {
  // Hard-disabled: WebGPU is the single biggest cause of "Aw, Snap!" tab
  // crashes on Android Chrome and on desktop Chrome with older drivers.
  // The WASM/CPU path is a little slower but never kills the tab.
  return false;
...
export function adaptiveWorkSize() {
  ...
  if (safeMode) return mobile ? 512 : 768;
  if (!mobile) return 1024; // PC / desktop always gets the model's best size.

  if (mem >= 6 || (mem === 0 && cores >= 8)) return 768; // high-end phone
  if (mem >= 4 || (mem === 0 && cores >= 6)) return 640; // medium phone
  if (mem > 0 && mem <= 2) return 512; // very low-end phone
  if (cores > 0 && cores <= 4) return 512;
  return 640;
}
...
/** Largest photo edge we keep in memory. Phones die on 2000px canvases. */
export function maxPhotoSize(preferred = 2000) {
  const mem = (typeof navigator !== "undefined" && navigator.deviceMemory) || 0;
  if (safeMode) return 1280;
  if (isMobileDevice()) return mem >= 6 ? 1600 : 1280;
  return preferred;
}
