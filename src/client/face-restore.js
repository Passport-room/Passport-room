...
import { markStart, markDone, withExclusiveRun } from "./crash-guard.js";
...
function runModel(ort, session, aligned) {
  ...
  // Rule R1 (crash-guard.js): bracket the GPU/WASM run with breadcrumbs.
  markStart();
  return session
...
/** One very light unsharp pass, only used when the face is far larger than
 *  256 px. Small amount, tight halo clamp — no crunch, no ringing. */
function lightSharpen(canvas, amount) {
  ...
  for (let i = 0; i < d.length; i += 4) {
    for (let ch = 0; ch < 3; ch++) {
      let delta = (d[i + ch] - bd[i + ch]) * amount;
      if (delta > cap) delta = cap;
      else if (delta < -cap) delta = -cap;
      d[i + ch] = clamp255(d[i + ch] + delta);
    }
  }
 ...
 */
export function restoreFace(source, opts = {}, onProgress) {
  // Rule R2 (crash-guard.js): one ONNX model at a time, always — this run
  // includes both the face detector and GPEN, so it must share the same
  // exclusive queue as background removal (never two sessions concurrently).
  return withExclusiveRun(() => restoreFaceInner(source, opts, onProgress));
}

async function restoreFaceInner(source, opts, onProgress) {
  ...
  if (upscaleRatio > 1.3) {
    // The only sharpening in the whole pipeline, and it stays subtle.
    lightSharpen(patch, Math.min(0.3, 0.14 * Math.log2(upscaleRatio + 1)));
  }
