...
if ($("saveCrop"))
  $("saveCrop").addEventListener("click", async () => {
    ...
    try {
      ...
      await window.__tryOn.applyResult(baked.toDataURL("image/png"), null, {
        w: baked.width,
        h: baked.height,
      });
...
window.__tryOn = {
  ...
  // background — used as the fixed base layer inside the Dress Editor.
  getPreviewDataUrl() {
    try {
  ...
  async applyResult(dataUrl, onStage, dims) {
    const img = new Image();
