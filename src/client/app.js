...
import { maxPhotoSize } from "./crash-guard.js";
...
// ---- Cached background cutout ------------------------------------------------
// Segmentation is the slowest step in the app. It is computed ONCE per uploaded
// photo and then reused for every later apply (AI Enhance, Outfit change,
// Revert). Only a genuinely new upload — or a bake that changes the framing
// itself (Save crop) — recomputes it.
let cachedMaskCanvas = null;
// Snapshots of the complete working state, so Revert restores instantly from
// cache instead of re-running the AI.
const stateStack = [];

function setCachedMask(maskCanvas) {
  cachedMaskCanvas = maskCanvas || null;
}

/** The cached mask, resampled when a tool changed the working image size. */
function cachedMaskFor(width, height) {
  if (!cachedMaskCanvas) return null;
  ...
  ctx.drawImage(cachedMaskCanvas, 0, 0, width, height);
  return c;
}
...
function restoreState(s) {
  cutout = s.cutout;
  originalCutoutCanvas = s.originalCutoutCanvas;
  editedCutoutCanvas = s.editedCutoutCanvas;
  lastSourceCanvas = s.lastSourceCanvas;
  cachedMaskCanvas = s.cachedMaskCanvas;
  backend = s.backend;
  timings = s.timings;
  renderBackendBadge();
  setPhase("result");
...
/** Big, unmissable failure banner over the preview image. A failure must never
 *  look like "the tool did nothing". */
function showPreviewBanner(msg) {
  const box = document.querySelector(".previewBox");
  if (!box) return;
  let el = document.getElementById("previewErrorBanner");
  if (!msg) {
  ...
  if (!el) {
    ...
    el.id = "previewErrorBanner";
    el.className = "previewErrorBanner";
...
/** Simple line sweep across the preview whenever a new image lands there. */
function playPreviewSweep() {
  const box = document.querySelector(".previewBox");
  if (!box) return;
  let sweep = document.getElementById("previewSweep");
  if (!sweep) {
    sweep = document.createElement("div");
    sweep.id = "previewSweep";
    sweep.className = "previewSweep";
...
function crystalAllowed() {
  // Only requirement: the device can actually make a WebGL context.
  // (The scene is tiny and the renderer caps its pixel ratio, so it is safe
  // on phones as well; ONNX/AI work is unaffected by this canvas.)
  try {
    const c = document.createElement("canvas");
...
function startFreshSession() {
  ...
  backend = null;
  timings = null;
  // A new photo is the ONLY thing that invalidates the cached cutout.
...
const downloadCloseBtn = $("downloadModalClose");
if (downloadCloseBtn)
  downloadCloseBtn.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    hideDownloadModal();
  });
...
window.__tryOn = {
  ...
  /**
   * Restores the working image that existed before the last applyResult().
   * Instant: it replays the cached cutout instead of re-running segmentation.
   * Falls back to a full apply if no snapshot is available.
   */
  async revertTo(dataUrl, onStage, dims, opts) {
    ...
    return this.applyResult(dataUrl, onStage, dims, opts);
  },
  ...
  async applyResult(dataUrl, onStage, dims, opts) {
    ...
    try {
      ...
      // Remember the state we are leaving so Revert is instant.
      if (cutout) {
        stateStack.push(snapshotState());
        if (stateStack.length > 8) stateStack.shift();
      }

      // Current working state only — originalUploadCanvas is never reassigned here.
      lastSourceCanvas = source;

      // Reuse the cutout computed for this photo. Segmentation only re-runs for
      // a genuinely new photo, or when the pixels were re-framed (Save crop).
      let maskCanvas = (opts && opts.reframe) ? null : cachedMaskFor(source.width, source.height);
      if (!maskCanvas) {
        onStage && onStage("Removing background…");
        const mask = await computeMask(source, () => {});
        maskCanvas = mask.maskCanvas;
        setCachedMask(maskCanvas);
        backend = mask.backend;
        timings = { inference: mask.inferenceMs, total: 0 };
      }

      cutout = composeCutout(source, maskCanvas, source.width, source.height);
      ...
      console.error(err);
      showPreviewBanner(
        (err && err.message) || "Something went wrong — your photo was not changed.",
      );
      throw err;
    }
  },
