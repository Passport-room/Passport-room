...
// Last background-removal mask for the active photo. Reused by the enhancer so
// enhancing never has to re-run (or lose) the cutout.
...
async function handleFile(file) {
  ...
  try {
    ...
    lastMaskCanvas = mask.maskCanvas;
...
window.__tryOn = {
  ...
  /**
   * Swaps in an enhanced version of the *current* photo.
   *
   * Unlike applyResult() this never re-runs background removal and never
   * re-frames the photo: the existing mask is reused (scaled to the new
   * resolution), so the picture keeps its background/cutout state and only
   * gains detail. Framing, crop and edits are preserved.
   */
  async applyEnhancedSource(dataUrl, onStage) {
    const img = new Image();
    img.crossOrigin = "anonymous";
    await new Promise((res, rej) => {
      img.onload = res;
      img.onerror = () => rej(new Error("Failed to load enhanced image"));
      img.src = dataUrl;
    });

    onStage && onStage("Updating your photo…");
    ...
    lastSourceCanvas = source;
    lastPersonDims = { w: source.width, h: source.height };

    if (lastMaskCanvas) {
      // Reuse the existing mask — same subject, just a sharper source.
      cutout = composeCutout(source, lastMaskCanvas, source.width, source.height);
      originalCutoutCanvas = cutout.canvas;
    } else if (cutout) {
      // No mask on record: keep the full photo (opaque mask, bbox recomputed).
      ...
      cutout = composeCutout(source, opaque, source.width, source.height);
      originalCutoutCanvas = cutout.canvas;
    } else {
      const mask = await computeMask(source, () => {});
      lastMaskCanvas = mask.maskCanvas;
