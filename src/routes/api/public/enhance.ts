...
4. Output ONLY the resulting enhanced HD image. Do not add any text commentary.`;
  ...
  const modelsToTry = [
    { model: "gemini-3.1-pro-preview", thinkingLevel: ThinkingLevel.HIGH },
    { model: "gemini-3.1-flash-lite-image", thinkingLevel: undefined },
    { model: "gemini-3.1-flash-image", thinkingLevel: undefined },
  ];
  ...
  for (const item of modelsToTry) {
    try {
      const response = await ai.models.generateContent({
        model: item.model,
        ...(item.thinkingLevel
          ? { config: { thinkingConfig: { thinkingLevel: item.thinkingLevel } } }
          : {}),
        contents: {
          parts: [
            {
      ...
      console.warn(
        `[enhance] Gemini model ${item.model} error:`,
