...
export async function upsertPaddleCustomer(input: {
  ...
  // Index so the account page can resolve a signed-in buyer's Paddle customer
  // id server-side (the client never supplies one).
  if (email) {
    await dbPut(`paddleCustomerByEmail/${emailKeyOf(email)}`, {
      customerId: input.id,
      updatedAt: Date.now(),
    }).catch(() => undefined);
  }
...
export async function upsertPaddleSubscription(input: {
  ...
  const customerKey = safeKey(record.customerId);
  if (customerKey) {
    await dbPut(`paddleCustomerSubscriptions/${customerKey}/${key}`, {
      status: record.status,
      updatedAt: record.updatedAt,
    }).catch(() => undefined);
  }
  if (record.email) {
    await dbPut(`paddleCustomerByEmail/${emailKeyOf(record.email)}`, {
...
/**
 * Does this mirrored subscription grant paid access right now?
 *
 * `active` and `trialing` grant access. A `scheduled_change` (cancel or pause
 * in the future) NEVER removes access — only the real status does:
 *   canceled -> no access
 *   paused   -> no access (the pause is already in effect)
 *   past_due -> access kept during Paddle's retry/dunning window
 */
export function subscriptionGrantsAccess(
  subscription: Pick<PaddleSubscriptionRecord, "status"> | null | undefined,
): boolean {
...
/** Paddle customer id mirrored for this email address (server-side lookup). */
export async function findPaddleCustomerIdByEmail(email: string): Promise<string | null> {
  if (!isEmail(email)) return null;
  const row = await dbGet<{ customerId?: string }>(
    `paddleCustomerByEmail/${emailKeyOf(email)}`,
  ).catch(() => null);
  return safeKey(row?.customerId);
}

/** Mirrored subscriptions belonging to a Paddle customer. */
export async function listSubscriptionsForCustomer(
  customerId: string,
): Promise<PaddleSubscriptionRecord[]> {
  const key = safeKey(customerId);
  if (!key) return [];
  const index = await dbGet<Record<string, unknown>>(
    `paddleCustomerSubscriptions/${key}`,
  ).catch(() => null);
  const ids = Object.keys(index ?? {});
  const rows = await Promise.all(ids.map((id) => readPaddleSubscription(id)));
  return rows.filter((r): r is PaddleSubscriptionRecord => Boolean(r));
}

/**
 * Mints a Paddle-hosted customer portal session. Paddle returns the URLs the
 * buyer uses to update payment details, cancel, or view invoices.
 * Read-only towards our own data; it creates nothing but the session itself.
 */
export async function createPaddlePortalSession(input: {
  customerId: string;
  subscriptionIds?: string[];
}): Promise<{ overviewUrl: string | null; manageUrl: string | null }> {
  const apiKey = (process.env["PADDLE_API_KEY"] ?? "").trim();
  if (!apiKey) throw new Error("PADDLE_API_KEY is not configured");

  const res = await fetch(
    `${paddleApiBase()}/customers/${encodeURIComponent(input.customerId)}/portal-sessions`,
    {
      method: "POST",
      headers: { Authorization: `Bearer ${apiKey}`, "content-type": "application/json" },
      body: JSON.stringify({ subscription_ids: input.subscriptionIds ?? [] }),
    },
  );
  if (!res.ok) throw new Error(`paddle portal session failed (${res.status})`);
  const body = (await res.json()) as {
    data?: {
      urls?: {
        general?: { overview?: string };
        subscriptions?: Array<{ cancel_subscription?: string; update_subscription_payment_method?: string }>;
      };
    };
  };
  ...
  return {
    ...
    manageUrl: sub?.update_subscription_payment_method ?? sub?.cancel_subscription ?? null,
  };
}
